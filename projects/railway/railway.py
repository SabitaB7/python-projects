...
class train:
    def __init__(self,name = '', num = 0, arr_time = '',dep_time = '',src = '' ,des = '',day_of_travel = '',seat_available_in_1AC = 0,seat_available_in_2AC = 0,seat_available_in_SL = 0,seat_available_in_GL = 0, fare_1ac = 0, fare_2ac = 0 , fare_sl = 0, fare_gl = 0):
        self.name = name
        self.num = num
        ...
        self.seats = {'1AC' : seat_available_in_1AC, '2AC': seat_available_in_2AC,'SL': seat_available_in_SL,'GL': seat_available_in_GL}
        self.fare = {'1AC' : fare_1ac,'2AC' : fare_2ac ,'SL' : fare_sl , 'GL' : fare_gl}
    def print_seat_availablity(self):
        ...
        print("No. of seats available in GL :- "+str(self.seats['GL']))
    def check_availabilty(self,coach='',ticket_num = 0):
        coach = coach.upper()
        if coach not in ('SL','1AC','2AC','GL'):
            print_seat_availablity()
            coach = input("Enter the coach(1AC/2AC/SL/GL) :-")
        else:
...
class acceptors:
	...
	def accept_pwd():
		try:
			...
			print("Please enter your password properly.")
			return acceptors.accept_pwd()
		else:
			return pwd
...
u1 = user('sandy','saru','syam','veera','abhi')
u2 = user('raji','mani','mari','syamala','sudheer')
